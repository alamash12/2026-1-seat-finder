...
function updateTimer() {
  ...
  if (reservation.status === "away") {
    const seconds = Math.max(0, Math.floor((reservation.awayUntil.toDate().getTime() - Date.now()) / 1000));
    timeLabel.textContent = "외출시간";
    remainTime.textContent = formatTime(seconds);
    updateTimerRing(seconds, AWAY_SECONDS);
    extendButton.disabled = true;
    return;
  }
...
function updateTimerRing(remainingSeconds, totalSeconds) {
  ...
  // SVG circle의 진행 방향을 반대로 뒤집어, 시간이 줄어들 때 시계방향으로 줄어들게 합니다.
